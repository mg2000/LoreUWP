				...
				async Task EndBattle()
				{
					...
					async Task CheckImperiumMinorEntraceBattleResult() {
						...
						if ((mParty.Etc[41] & (1 << 6)) == 0)
						{
							...
							" 원래 거기는 에이션트 이블이 전에 세운 악의 동굴이었지만 네크로만서의 침략으로 지상의 도시가 함락되자 그 곳의 사람들은 모두 거기로 피난 했던 것이고" +
							" 거기는 에이션트 이블의 영적인 힘으로 보호되고 있어서 적들이 침략을 하지 못하는 이유가 되지요. 그러므로 모든 도움과 물자는 거기서 받도록 하시오.",
							" 그러면, 나는 당신이 네크로만서와 상대하게 될 때 다시 에이션트 이블과 같이 나타나겠소."});
					...
					async Task InvokeSpecialEventLaterPart()
					{
						...
						else if (mSpecialEvent == SpecialEventType.SwampGatePyramid2)
						{
							Talk(new string[] {
								...
								$"[color={RGB.White}] 여기서 당신은 로드 안, 에이션트 이블, 네크로만서의 관계를 기술한 예언서를 발견하여 읽기 시작했다.[/color]"
							});
						...
						else if (mSpecialEvent == SpecialEventType.SwampGatePyramid4)
						{
							Talk(new string[] {
								...
								$"",
								" 만약 당신이 황야에서 에이션트 이블을 만나더라도 두려워하지 말라. 그는 비록 악의 표상이지만 네크로만서가 행하는 악과는 다른 표현임을 명심하라." +
						...
						else if (mSpecialEvent == SpecialEventType.SwampGatePyramid5)
						{
							Talk(new string[] {
								...
								$"",
								" 위에서 기술한 에이션트 이블이 의미하는 악과는 달리 네크로만서는 진정한 악의 의미를 알지 못한다. 그것으로 인해 에이션트 이블은 그를 벌하려 하는 것이다." +
								" 하지만 육체가 없어진 에이션트 이블의 능력으로는 그에게 대항하기가 어렵다고 단정하고는 그의 강력한 마력으로 미래의 역사를 뒤틀어 운명적으로 네크로만서에" +
						...
						else if (mSpecialEvent == SpecialEventType.SwampGatePyramid6)
						{
							Talk(new string[] {
								...
								$"",
								" 네크로만서에게 대항할 수 있는 단 두 명의 존재는 바로 로드 안과 그의 대립자이며 깊은 관계를 가진 에이션트 이블이다.",
