...
namespace Lore
{
	...
	public sealed partial class NewGamePage : Page
	{
		private QuestionInfo[] mQuestionList = {
			new QuestionInfo("당신이 한밤중에 공부하고 있을 때 밖에서 무슨 소리가 들렸다",
				new string[] {
					"1] 밖으로 나가서 알아본다",
					"2] 그 소리가 무엇일까 생각을 한다",
					"3] 공부에만 열중한다"
			...
			new QuestionInfo("당신은 이 게임 속에서 적들에게 완전히 포위되어 승산 없이 싸우고\r\n있다",
				new string[] {
					"1] 힘이 남아 있는 한 죽을 때까지 싸운다",
					"2] 한 가지라도 탈출할 가능성을 찾는다",
					"3] 일단 싸우면서 여러 방법을 생각한다"
				},
				new int[] { 0, 1, 4 }),
			new QuestionInfo("당신은 매우 복잡한 매듭을 풀어야 하는 일이 생겼다",
				new string[] {
					"1] 칼로 매듭을 잘라 버린다",
					"2] 매듭의 끝부분부터 차근차근 훑어본다",
					"3] 어쨌든 계속 풀려고 손을 놀린다"
				},
				new int[] { 0, 2, 3 }),
			new QuestionInfo("허허벌판을 걸어가던 당신은 갑작스러운 우박을 만난다",
				new string[] {
					"1] 당황한 나머지 피할 곳을 찾아 뛴다",
					"2] 침착하게 주위를 살펴 안전한 곳을 찾는다",
					"3] 우박 정도는 그냥 견딘다"
				},
				new int[] { 0, 2, 4 }),
			new QuestionInfo("집안에 불이 나서 탈출하려는데 나무로 만든 문이 좀처럼 열리지\r\n않는다",
				new string[] {
					"1] 다른 탈출구를 찾아간다",
					"2] 1] 번과 같은 불확실한 도전을 하는 것보다는 확실한\n 탈출구인 이 문을 끝까지 열려 한다",
					"3] 나무 문이 타서 구멍이 생길 때까지 기다려 탈출한다"
				},
				new int[] { 0, 3, 4 }),
			...
			new QuestionInfo("시험 기간에 당신이 도서관에서 공부를 하려는데 주위가 너무\r\n시끄럽다",
				new string[] {
					"1] 상관없이 참으며 공부한다",
					"2] 너무 공부를 열심히 해서 그런 소리가 안 들린다",
					"3] 시끄러움에 대항하는 마음으로 공부한다"
				},
				new int[] { 1, 2, 4 }),
			new QuestionInfo("직장 생활을 하던 당신은 아무 이유 없이 상관에게 심한 욕을 들었다",
				new string[] {
					"1] 겉으로는 순종하면서 속으로는 감정을 쌓는다",
					"2] 웬만하면 참고 넘긴다",
					"3] 상관인 걸 무시하고 이유를 들라며 대든다"
				},
				new int[] { 1, 3, 4 }),
			new QuestionInfo("당신이 새로운 프로그램을 짜던 중 알 수 없는 오류가 생겼다",
				new string[] {
					"1] 차근차근 순서도를 생각하며 오류를 찾는다",
					"2] 여러 번 실행시키며 오류를 찾는다",
					"3] 오류가 작으면 그냥 사용한다"
				},
				new int[] { 2, 3, 4 }),
